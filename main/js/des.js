 ...
 * 加密
 */
function encryptDes(data, firstKey, secondKey, thirdKey) {
 ...
 * 解密
 */
function decryptDes(data, firstKey, secondKey, thirdKey) {
	var leng = data.length;
	var decStr = "";
	var firstKeyBt, secondKeyBt, thirdKeyBt, firstLength, secondLength, thirdLength;
	if (firstKey != null && firstKey != "") {
...
/**
 * chang the string(it's length <= 4) into the bit array return bit array(it's
 * length = 64)
 */
...
function encryptCode(data, key, datetime) {
	data = data == null ? "" : $.trim(data);
	key = key == null ? "" : $.trim(key);
	datetime = datetime == null ? "" : $.trim(datetime);
	if (data=="" || key==""|| datetime=="")
		return ("");
	var sub_time = datetime.substring(datetime.length - 1, datetime.length);
	var position = 0;
	try {
		position = parseInt(sub_time);
		if (position == 0)// 0
		{
			data = encryptDes(data, key, datetime, key);
			data = encodeBase64(data);
			return (data);
		}
		if ((position % 2) == 1)// 奇数
		{
			data = encryptDes(data, key, key, datetime);
			data = encodeBase64(data);
			return (data);
		}
		if ((position % 2) == 0)// 偶数
		{
			data = encryptDes(data, datetime, key, datetime);
			data = encodeBase64(data);
			return (data);
		}
	} catch (e) {
...
function decryptCode(data, key, datetime) {
	data = data == null ? "" : $.trim(data);
	key = key == null ? "" : $.trim(key);
	datetime = datetime == null ? "" : $.trim(datetime);
	if (data=="" || key=="" || datetime=="")
	...
	try {
		...
		if (position == 0)// 0
		{
			...
			data = decryptDes(data, key, datetime, key);
			return (data);
		}
		if ((position % 2) == 1)// 奇数
		{
			data =decodeBase64(data);
			data = decryptDes(data, key, key, datetime);
			return (data);
		}
		if ((position % 2) == 0)// 偶数
		{
			data =decodeBase64(data);
			data = decryptDes(data, datetime, key, datetime);
			return (data);
		}
	} catch (e) {
